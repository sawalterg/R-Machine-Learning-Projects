...
prob_pred <- predict(classifier, type = 'response', newdata = test_set[-3]) # We don't want the dependent variables involved in the prediction
...
point(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato')) # Pixel results for imaginary users (the whole grid)
