...
# 2. Hyperparameters are the ones we choose (cost 'C',kernel parameters (used to improve the model)). Since these are not learned by the models, we need to optimize how we select these parameters
...
# Grid search helps determine the ideal hyperparameters without guess work/tinkering
...
# You can search the github link by searching caret in google
# caret package is ideal for parameter tuning
# Here we want the svm with radial basis function (the gaussian kernel)
...
classifier$bestTune
# This can be obtained by executing the classifier or pulling the bestTune method
# The optimal values are at the end for the sigma and C
